...
tld = 'C:\Users\arrio\Box\BensmaiaLab\UserData\UserFolders\ToriArriola\DARPA_updated\PreProcessedData\Pinot\Cathodic_Anodic';
...
subplot(1,3,2); hold on
   ...
   text( .03, .7,'0.021', 'Color', rgb(66, 66, 66),'FontSize',18);
   text(.03, 1, '0.091', 'Color', rgb(198, 40, 40), 'FontSize',18);
   ylabel('d''')
   xlabel('Stimulus Amplitude (mm)')
axis square
...
w_o_icms_cath = vertcat(data(cath_idx).woicmsthreshold);
w_icms_cath = vertcat(data(cath_idx).wicmsthreshold);
...
for d1 = 1:length(data)
    ...
    scatter(w_icms_an, w_o_icms_an,100,rgb(48, 63, 159), 'filled')
...
text(.2,.1, 'Cathodic', 'Color',rgb(123, 31, 162))
text(.2,.08, 'Anodic', 'Color',rgb(48, 63, 159))
%    text( .03, .7,'0.021', 'Color', rgb(66, 66, 66),'FontSize',18);
...
ylabel('\Delta Cathodic Threshold')%'\Delta Predicted \muA - Observed \muA
